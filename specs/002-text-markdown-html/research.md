...
- **兩個數字門檻的用途區分**（避免與 SC-001 混淆）：
  - **20,000 字元＝UI 體驗門檻**：低於此門檻，轉換在同一個 macrotask 內同步完成並直接顯示結果；超過此門檻時，系統**仍會自動即時轉換**（不違反 FR-003「無需按下轉換按鈕」），但會先顯示「處理中」狀態、以 `setTimeout(0)` 讓出一次繪製機會，避免長字串運算卡住畫面掉幀。
  - **50,000 字元＝SC-001 效能驗收上限**：為此功能的效能驗收基準與測試涵蓋範圍（見 tasks.md T027）——在此字元數以內，轉換 MUST 於 1 秒內完成且不當機、不逾時；超過此上限則不在 v1 的效能保證範圍內（非阻斷輸入，僅效能不再保證）。
